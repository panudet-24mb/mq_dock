...
def connect():
    DBconnect = MySQLdb.connect(
        host="db", user="devops", passwd="devops101", db="devops_db", port=3306
    )
    return DBconnect
...
def insert(id, firstname, lastname, subjectid, term, year):
    ...
    cur.execute(
        "INSERT INTO enroll (id, name, subjectid, term, year) VALUES (%s, %s, %s, %s, %s);",
        (id, firstname + " " + lastname, subjectid, term, year),
    )
    id = cur.lastrowid
    DBconnect.commit()
    DBconnect.close()
